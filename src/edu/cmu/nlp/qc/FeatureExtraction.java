...
public class FeatureExtraction {
	...
	public static String extractFeatures(Question question){
		...
		if(WhWord.whMap.containsKey(whWord)){
			whVec.set(WhWord.whMap.get(whWord));
		}
		String whFV = whVec.toString();
		
		BinaryFeatureVector wordVec = new BinaryFeatureVector(Dict.getSize());
		String headWord = question.getParsingTree().headTerminal(headFinderLK).toString();
		if(Dict.getWordIdx(headWord) >= 0)
			wordVec.set(Dict.getWordIdx(headWord));
		...
		for(String hypernym : hypernyms){
			if(Dict.getWordIdx(hypernym) >= 0)
				wordVec.set(Dict.getWordIdx(hypernym));
		}
		//Add unigram
		for(Chunk chunk : question.getTokens()){
			if(Dict.getWordIdx(chunk.getWord()) >= 0)
				wordVec.set(Dict.getWordIdx(chunk.getWord()));
		}
